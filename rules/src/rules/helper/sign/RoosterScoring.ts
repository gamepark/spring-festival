...
export class RoosterScoring extends SignScoring {
  ...
  get extinguishCount() {
    ...
    const exploded = this.panorama.filter<FireworkId>((item) => fireworkDescriptions[item.id.front].explosions.length === 3)
    for (const index of exploded.getIndexes()) {
      const item = exploded.getItem<FireworkId>(index)
